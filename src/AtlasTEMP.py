...
def output(args:argparse.Namespace, measurement:str="NONE", unit:str="f", showdeviceinfo:bool=False, deviceinfo:str="NONE"):
	_timestamp=datetime.datetime.now(datetime.timezone.utc).isoformat()
	if (showdeviceinfo):
		_readingtemplate='{{"timestamp":"{time}", "ChamberTemp":"{temp}", "Unit":"{unit}", "Device":"{dev}"}}'
		formated_measurement=_readingtemplate.format(time=_timestamp, temp=measurement, unit=unit, dev=deviceinfo)
	else:
...
def main():
	...
	parser.add_argument("--logfile", default="./AtlasTEMP_UF", help="Sets file path into which output measurements are appended. Only used with --output=LOG.   (Default: ./AtlasTEMP_UF.log)")
	...
	if (args.listentime==-1):
		...
		while True:
			...
			output(args, measurement, args.unit.upper(), args.show_device_info, device_info)
